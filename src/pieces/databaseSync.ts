...
async function register(bot: Client): Promise<void> {
	handleCron(bot);
	schedule("0 3 * * *", () => {
		// run every day at 3:00am (time chosen because of low activity)
...
async function handleCron(bot: Client) {
	...
	guild.members.cache.forEach(async (member) => {
		...
		const currentUser = await bot.mongo
			.collection<SageUser>(DB.USERS)
			.findOne({ discordId: member.user.id });
		...
		member.roles.cache.forEach((role) => {
			if (role.name !== "@everyone") {
				...
				if (role.name.match(/CISC .{1,}/g)) {
					// checks if the role name starts with "CISC " and is followed by one or more additional characters
