...
export default class extends Command {
    ...
    async run(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const classOptions = [
            'CISC106', 'CISC108', 'CISC181', 'CISC210', 'CISC220', 'CISC260', 'CISC275', 'CISC000'
        ];

        const selectMenu = new StringSelectMenuBuilder()
            .setCustomId('class_menu')
            .setPlaceholder('Select Class')
            .addOptions(classOptions.map(className => ({
                label: className,
                value: className
            })));
        ...
        const filter = (i: StringSelectMenuInteraction) => i.customId === 'class_menu' && i.user.id === interaction.user.id;
        ...
        collector.on('collect', async (i: StringSelectMenuInteraction) => {
            ...
            if (className && !classNameRegex.test(className)) {
                await i.reply({
                    content: 'Invalid class name format. Please select a valid class name.',
                    ephemeral: true, // Only visible to the user who entered the command
                });
                return;
            }

            async function listEvents(auth, interaction: StringSelectMenuInteraction, className: string) {
                ...
                try {
                    ...
                    if (events.length === 0) {
                        await i.editReply(`No TAs found for course: **${className}**. Please check back later or contact the instructor.`);
                        return;
                    }

                    // Filter events by class name
                    const filteredEvents = events.filter((event) => {
                        return event.summary && event.summary.toLowerCase().includes(className.toLowerCase());
                    });
                    ...
                    const eventHolders = Array.from(
                        new Set(filteredEvents.map((event) => event.summary.split("-")[1]?.trim()))
                    ).filter(Boolean);
                    ...
                    // Send the list of TAs to the user
                    await i.editReply({
