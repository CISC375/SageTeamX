...
export default class CalendarCommand extends Command {
	...
	description = `Provides info about all ${BOT.NAME} calendar commands`;
	...
	async run(interaction: ChatInputCommandInteraction): Promise<InteractionResponse<boolean> | void> {
		...
		if (cmd) {
			...
			*/
			...
			// if no command given
			let helpStr = `You can do \`/calendar <command>\` to get more information about any command, or you can visit our website here:\n<${website}>\n`;
			...
			categories.forEach(cat => {
				...
				const categoryName = cat === 'commands' ? 'Calendar' : `${cat[0].toUpperCase()}${cat.slice(1)}`;
			...
			splitStr.forEach((helpMsg) => {
				...
				interaction.user.send({ embeds: [embed] })
					.then(() => {
						if (!notified) {
							interaction.reply({ content: 'I\'ve sent all calendar commands to your DMs.', ephemeral: true });
							notified = true;
						}
					})
					.catch(() => {
